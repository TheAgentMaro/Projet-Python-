...
class game:
    ...
    def propagation(self ,coord: tuple, liste: list):   # fonction pour calculer toutes les cases adjacentes d'une cellule
            ...
            if coord[0] - 1 >= 0:
                if self.matrice[coord[0] - 1][coord[1]] == self.matrice[coord[0]][coord[1]]:
                    if (coord[0] - 1, coord[1]) not in liste:
                        ...
                        self.propagation(size, self.matrice, (coord[0] - 1, coord[1]), liste)
            if coord[0] + 1 < size:
                if self.matrice[coord[0] + 1][coord[1]] == self.matrice[coord[0]][coord[1]]:
                    if (coord[0] + 1, coord[1]) not in liste:
                        liste.append((coord[0] + 1, coord[1]))
            ...
            if coord[1] - 1 >= 0:
                if self.matrice[coord[0]][coord[1] - 1] == self.matrice[coord[0]][coord[1]]:
                    if (coord[0], coord[1] - 1) not in liste:
                        ...
                        self.propagation(size, self.matrice, (coord[0], coord[1] - 1), liste)
            if coord[1] + 1 < size:
                if self.matrice[coord[0]][coord[1] + 1] == self.matrice[coord[0]][coord[1]]:
                    if (coord[0], coord[1] + 1) not in liste:
                        liste.append((coord[0], coord[1] + 1))
    ...
    def modification(self , liste: list) : # fonction incrementer et pour mettre un 0 sur toutes cases adjacentes qu'on a trouvé
