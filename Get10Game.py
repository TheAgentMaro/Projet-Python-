...
class game:
        ...
        for i in range(size):
            for j in range(size):
                if not 0 <= i < size or not 0 <= j < size:
                    return 'Error'
                if i + 1 < self.size:
                    if self.matrice[i + 1][j] == self.matrice[i][j]:
                        return True
                if j + 1 < self.size:
                    if self.matrice[i][j + 1] == self.matrice[i][j]:
                        return True
                if 0 <= j - 1:
                    if self.matrice[i][j - 1] == self.matrice[i][j]:
                        return True
    ...
    def reste_coup(self):
        # on debut on a rien trouvé on sait pas si une case a une meme valeur adjacente
        resultat = False
        for i in range(size):
            for j in range(size):
    ...
    # fonction pour calculer toutes les cases adjacentes d'une cellule
    def propagation(self, coord: tuple, liste: list):
        if 0 <= coord[0] < size and 0 <= coord[1] < size:
            if coord[0] - 1 >= 0:
                if self.matrice[coord[0] - 1][coord[1]] == self.matrice[coord[0]][coord[1]]:
                    if (coord[0] - 1, coord[1]) not in liste:
                        liste.append((coord[0] - 1, coord[1]))
                        self.propagation(size, self.matrice,
                                         (coord[0] - 1, coord[1]), liste)
            if coord[0] + 1 < size:
                if self.matrice[coord[0] + 1][coord[1]] == self.matrice[coord[0]][coord[1]]:
                    if (coord[0] + 1, coord[1]) not in liste:
                        liste.append((coord[0] + 1, coord[1]))
                        self.propagation(size, self.matrice,
                                         (coord[0] + 1, coord[1]), liste)
            if coord[1] - 1 >= 0:
                if self.matrice[coord[0]][coord[1] - 1] == self.matrice[coord[0]][coord[1]]:
                    if (coord[0], coord[1] - 1) not in liste:
                        liste.append((coord[0], coord[1] - 1))
                        self.propagation(size, self.matrice,
                                         (coord[0], coord[1] - 1), liste)
            if coord[1] + 1 < size:
                if self.matrice[coord[0]][coord[1] + 1] == self.matrice[coord[0]][coord[1]]:
                    if (coord[0], coord[1] + 1) not in liste:
                        liste.append((coord[0], coord[1] + 1))
    ...
    # fonction incrementer et pour mettre un 0 sur toutes cases adjacentes qu'on a trouvé
    def modification(self, liste: list):
        self.matrice[liste[0][0]][liste[0][1]] += 1
        for i in range(1, len(liste)):
            self.matrice[liste[i][0]][liste[i][1]] = 0

    # pour deplacer les cases vers le bas s'il ya des 0 et ajouter de nouvelles valeurs aleatoires
    def gravity(self):
        for i in range(size):
            for j in range(size):
        ...
        for p in range(size):
            for m in range(size):
                if self.matrice[p][m] == 0:
                    # du coup la derniere case en haut n'aura rien et on y met une valeur aleatoire
                    self.matrice[p][m] = self.aleatoire(self)
...
class GUI:
    ...
    def Leftclick(self, event):
        x, y = event.x, event.y
        print('{}, {}'.format(x, y))
        # calcule la longueur de la grille + sa marge gauche
        taillex = GUI.MARGIN + (self.game.size * self.label.grid)
        # calcule la hauteur de la grille + sa marge haut
        tailley = GUI.MARGIN + (self.game.size * self.label.grid)
        # test pour connaitre si on a cliqué a l'interieur de la grille
        if x >= 0 and x < taillex and y >= 0 and y < tailley:
            # on calcule a partir du y de la souris(qui est en pixel ) le i de la case selectionnée
            i = int((y - GUI.MARGIN)/self.label.grid)
            j = int((x - GUI.MARGIN) / self.label.grid)
            # c'est la case selectionée
            current = (i, j)
            L = [current]
            print(L)
    ...
    def click(self):
        if self.game.existe_case_adj():
            # appelle procedure modification pour incrementer et mettre les 0
            self.game.modification()
